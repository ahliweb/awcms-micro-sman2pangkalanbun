...
const files = ["news.sample.json", "pages.sample.json", "finance-reports.sample.json"];
...
function existsInLegacy(p) {
	...
	return (
		fs.existsSync(path.join(legacyRoot, "public", p.slice(1))) ||
		fs.existsSync(path.join(legacyRoot, p.slice(1)))
	);
}
		...
		for (const row of item.localeRows || []) {
			...
			if (row.heroImageSource && !existsInLegacy(row.heroImageSource)) {
				unresolved.push({
					file: name,
					slug: row.slug,
					field: "heroImageSource",
					value: row.heroImageSource,
				});
			}
			for (const attachment of row.attachments || []) {
				if (attachment.fileSource && !existsInLegacy(attachment.fileSource)) {
					unresolved.push({
						file: name,
						slug: row.slug,
						field: "attachments.fileSource",
						value: attachment.fileSource,
					});
				}
			}
		}
