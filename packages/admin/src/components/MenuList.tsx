...
export function MenuList() {
	...
	const createMutation = useMutation({
		...
		onSuccess: (menu) => {
			...
			void navigate({ to: "/menus/$name", params: { name: menu.name }, search: { locale: undefined } });
		},
		onError: (error: Error) => {
			...
			{!menus || menus.length === 0 ? (
				...
				<div className="grid gap-4">
					{menus.map((menu) => (
						...
						>
							<Link to="/menus/$name" params={{ name: menu.name }} search={{ locale: undefined }} className="flex-1">
								<div>
